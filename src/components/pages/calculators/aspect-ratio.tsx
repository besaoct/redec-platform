...
export default function AspectRatioCalculator() {
  ...
  const share = () => {
    ...
    if (navigator.share) {
      navigator
        .share({ title: "Aspect Ratio", text, url })
        .catch(() => {
    ...
    <div className="max-w-5xl mr-auto animate-fade-in">
      <div className="mb-6">
        <h1 className="text-xl sm:text-3xl font-extrabold font-display">
          Aspect Ratio <span className="text-primary">Calculator</span>
        </h1>
        <p className="text-muted-foreground mt-2">
          Calculate dimensions, ratios, and common screen sizes
        </p>
      </div>
      ...
      <div className="mt-8 bg-muted/30 p-6 rounded border">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          ...
          Enter any two values (width, height, or ratio) and the rest will be calculated automatically.
          The preview shows how your current ratio would appear on screen.
        </p>
      </div>
    </div>
  );
