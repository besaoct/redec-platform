...
import { VIBES, NICKNAME_TEMPLATES } from "@/data/tools/nickname-generator";
...
export default function NicknameGenerator() {
  ...
  return (
    <div className="max-w-5xl mr-auto animate-fade-in">
      <div className="mb-6">
        <h1 className="text-xl sm:text-3xl font-extrabold font-display">Nickname <span className="text-primary">Generator</span></h1>
        <p className="text-muted-foreground mt-2">Create unique and fun nicknames based on your vibe</p>
      </div>
        ...
        <CardContent className="pt-6">
          <div className="grid gap-6">
            ...
            <div className="flex gap-3">
              <Button type="submit" className="flex-1" onClick={handleGenerate} disabled={!name || !selectedVibe}>
        ...
        <div className="grid gap-4 animate-in slide-in-from-bottom-4 duration-500">
           ...
           <div className="grid gap-3">
              {nicknames.map((nick, idx) => (
                <Card key={idx} className="group hover:border-primary/50 transition-all overflow-hidden relative">
                   <CardContent className="p-4 flex items-center justify-between">
                      ...
                      >
                        {copiedIndex === idx ? <Check className="h-4 w-4 text-success" /> : <Copy className="h-4 w-4 text-muted-foreground" />}
                      </Button>
                   </CardContent>
                </Card>
              ))}
      ...
      <div className="mt-8">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          The Nickname Generator uses your chosen vibe as a template engine. It applies specific naming conventions associated with that style to your name. The results are deterministic, meaning you'll get a consistent set of aliases every time you use the same name and vibe!
        </p>
      </div>
    </div>
  );
