...
import { Users, RefreshCcw, Share2, Check, ShieldAlert, Zap, Coffee, Gamepad2, Ghost, Rocket, User } from "lucide-react";
import { cn } from "@/lib/utils";
import { toast } from "sonner";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import { Progress } from "@/components/ui/progress";
// Fallback icon for Cosmic Twin since Sparkles isn't imported from lucide-react above
import { Sparkles as CosmicIcon } from "lucide-react"; 
...
export default function BondTester() {
  ...
  const calculateBond = useCallback((n1: string, n2: string) => {
    ...
    const combined = (n1.toLowerCase().trim() + n2.toLowerCase().trim() + "friendship_salt").split("").sort().join("");
    let hash = 0;
  ...
  const getBondData = (pct: number) => {
    return BOND_DATA.find(b => pct >= b.scoreRange[0] && pct <= b.scoreRange[1]) || BOND_DATA[0];
  };
  ...
  return (
    <div className="max-w-5xl mr-auto animate-fade-in">
      <div className="mb-6">
        <h1 className="text-xl sm:text-3xl font-extrabold font-display">Bond <span className="text-primary">Tester</span></h1>
        <p className="text-muted-foreground mt-2">Analyze the deep, algorithmic nature of your friendship</p>
      </div>
      ...
      {score !== null && result && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 duration-500">
          ...
          {/* Main Result Card */}
          <Card className={cn("lg:col-span-2 relative overflow-hidden border-2", result.color.replace('text-', 'border-').replace('500', '500/20'))}>
            <div className={cn("absolute inset-0 opacity-20 blur-3xl", result.bg)} />
            
             <div className="absolute top-4 right-4 z-10">
               <Tooltip open={copied}>
                  <TooltipTrigger asChild>
                    <Button variant="outline" size="icon" className="h-8 w-8 rounded-full bg-background/50 backdrop-blur-sm" onClick={share}>
            ...
            <CardContent className="text-center py-10 relative z-10 flex flex-col items-center">
              ...
              <div className="text-6xl num mb-2 tracking-tighter">
                {score}%
              </div>
              
              <div className="w-full max-w-md mx-auto mb-6 ">
                <Progress value={score} className="h-2 bg-muted" indicatorClassName={result.bg.replace('/10', '')} />
              </div>

              <div className="text-3xl mb-3">{result.emoji}</div>
              <CardTitle className={cn("text-3xl font-bold mb-4 uppercase tracking-wider font-display", result.color)}>
                {result.type}
              </CardTitle>
          ...
          <Card className="h-full flex flex-col w-full">
             ...
             <CardContent className="pt-6 flex-1 flex flex-col gap-6">
                ...
                <div>
                  <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3">Key Traits</h4>
                  <div className="flex flex-wrap gap-2">
                    {result.traits.map(trait => (
                      <span key={trait} className={cn("px-2.5 py-1 rounded-md text-xs font-medium border", result.bg, result.color.replace('text-', 'border-').replace('500', '500/30'))}>
                        {trait}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex-1">
                  <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3">Suggested Activities</h4>
      ...
      <div className="mt-8 bg-muted/30 p-4 rounded-xl">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          The Bond Tester uses a sophisticated (and entirely fictional) cross-referencing hash function. It takes the specific character composition of both names, applies a friendship salt, and calculates a permanent resonance score. Different score brackets unlock distinct friendship archetypes, complete with behavioral traits and activity suggestions.
        </p>
      </div>
    </div>
  );
